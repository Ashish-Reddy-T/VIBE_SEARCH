...
EMBEDDING_MODEL = os.getenv("EMBEDDING_MODEL", "minilm")  # Default embedding model

# Paths for data files
INDEX_FILE = os.path.join(BASE_DIR, "ingestion", f"index_{EMBEDDING_MODEL}.faiss")
METADATA_FILE = os.path.join(BASE_DIR, "ingestion", f"metadata_{EMBEDDING_MODEL}.pkl")
...
try:
    model_name = embedding_model_map.get(EMBEDDING_MODEL, "all-MiniLM-L6-v2")
...
def search_places(query: str, limit: int = 20, neighborhood_filter: Optional[str] = None, 
                 vibe_filters: Optional[List[str]] = None, timeout: float = 5.0) -> List[Dict]:
